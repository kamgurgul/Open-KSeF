...
/**
 * iOS [InvoicePdfExporter] placeholder. PDF export is intentionally disabled on iOS for now;
 * [isSupported] is `false` so the UI hides the export action on this platform.
 */
class IosInvoicePdfExporter : InvoicePdfExporter {
    ...
    override suspend fun export(invoiceXml: String, ksefReferenceNumber: String): PdfExportResult =
        PdfExportResult.Failure("PDF export is not supported on iOS yet")
}

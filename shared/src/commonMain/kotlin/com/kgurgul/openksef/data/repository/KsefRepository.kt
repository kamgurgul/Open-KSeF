...
class KsefRepository(
    ...
    /**
     * Runs the full token-based authentication flow:
     *   GET /security/public-key-certificates → POST /auth/challenge → encrypt(`token|timestampMs`)
    ...
    suspend fun initSession(nip: String, ksefToken: String): Result<SessionInfo> = runCatching {
        ...
        val encryptedToken = Base64.encode(
            crypto.rsaOaepSha256Encrypt(payload, certificateDer)
        )

        val initResponse = api.initTokenAuthentication(
    ...
    private fun pickKsefTokenEncryptionCertificate(
        ...
        return candidates.firstOrNull()
            ...
            ?: error("No KsefTokenEncryption certificate returned by /security/public-key-certificates")
    }
    ...
    private suspend fun waitForRedeem(authReferenceNumber: String): AuthenticationTokensResponse {
        repeat(MAX_AUTH_POLL_ATTEMPTS) {
            ...
            when (status.status.code) {
                AUTH_STATUS_SUCCESS -> return api.redeemAccessToken(
                ...
                else -> error("Authentication failed: ${status.status.code} ${status.status.description}")
            }
        }
    ...
    suspend fun sendInvoice(invoiceXml: String): Result<SendInvoiceResult> = runCatching {
        val sessionRef = sessionHolder.onlineSessionReferenceNumber
    ...
    suspend fun openOnlineSession(
        ...
        val response = api.openOnlineSession(
    ...
    private fun InvoiceMetadata.toSummary(): InvoiceSummary = InvoiceSummary(
